...
class ScopedStatus final {
 public:
  ...
  explicit ScopedStatus(std::weak_ptr<MainThreadExecutor> main_thread_executor, StatusListener* status_listener,
  ...
  void UpdateMessage(const std::string& message) {
    ...
    if (std::this_thread::get_id() == data_->main_thread_id) {
      data_->status_listener->UpdateStatus(data_->status_id, message);
		...
		auto executor = data_->main_thread_executor.lock();
