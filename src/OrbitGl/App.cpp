      ...
      .ThenIfSuccess(
          ...
          [this, module,
           function](const std::filesystem::path& local_file_path) -> ErrorMessageOr<void> {
            ...
            const auto first_line_or_error = elf_file.value()->GetLineInfo(function.address());

            if (first_line_or_error.has_error()) {
              return ErrorMessage{absl::StrFormat(
                  "Could not find source code line info for function \"%s\" in module \"%s\": %s",
                  function.pretty_name(), module->file_path(),
                  first_line_or_error.error().message())};
            }
            const auto& line_info = first_line_or_error.value();
