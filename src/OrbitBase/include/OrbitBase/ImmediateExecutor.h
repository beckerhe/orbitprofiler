...
// Future<void> void_future = result.Then(&immediate_executor, [](const std::string& str) { (void)
// str; }); main_thread_executor_->WaitFor(void_future); // WaitFor only works with Future<void>
//
 ...
 public:
  ...
  auto ScheduleAfterIfSuccess(const Future<ErrorMessageOr<T>>& future, F&& invocable) {
    ...
    auto continuation = [invocable = std::forward<F>(invocable),
      ...
      if (result.has_error()) {
        promise.SetResult(result.error());
        return;
      }
      orbit_base::CallTaskAndSetResultInPromise<ReturnType> helper{&promise};
      helper.Call(invocable, result.value());
    };
