...
function create_conan_profile {
  ...
  local readonly compiler_version="$(conan profile show default | grep compiler.version= | cut -d= -f2 | sed -e 's/^15$/2017/' | sed -e 's/^16$/2019/')"
...
for profile in ${profiles[@]}; do
  ...
  conan install -u -pr:b $build_profile -pr:h $profile -if build_$profile/ --build outdated $DIR || exit $?
